...
class sniffer {  
	...
	elseif ( eregi( 'Firefox/([0-9].[0-9a-zA-Z]{1,4})',$this->UA,$info) ) 
	{  $this->VERSION = $info[1];
	 $this->NAME = 'Firefox';
	}
	elseif ( eregi( 'Mozilla/([0-9].[0-9a-zA-Z]{1,4})',$this->UA,$info) ) 
	{  $this->VERSION = $info[1];
	 $this->NAME = 'Mozilla compatible';
	}
	else 
	{  $this->VERSION = 0;
	 $this->NAME = 'Unknown';
	...
	elseif(eregi('WinNT ([0-9].[0-9a-zA-Z]{1,4})',$this->UA,$info) || eregi('windows NT ([0-9].[0-9a-zA-Z]{1,4})',$this->UA,$info)) 
	{  $this->PLATFORM = "Win32"; 
	 if ($info[1] > 4) {
	 	$this->OS = "Windows XP/2000"; 
	 }
	 else {
	 	$this->OS = "Windows NT" . $info[1]; 
	 }
	}
	else
	{  $this->PLATFORM = "Win32"; 
...
echo 'MAJORVER=' . $is->MAJORVER . '<BR>';
echo 'MINORVER=' . $is->MINORVER . '<BR>';
echo 'AOL=' . $is->AOL . '<BR>';
echo 'WEBTV=' . $is->WEBTV . '<BR>';
echo 'JS=' . $is->JS . '<BR>';
echo 'PLATFORM=' . $is->PLATFORM . '<BR>';
echo 'OS=' . $is->OS . '<BR>';
echo 'IP=' . $is->IP . '<BR>';  
*/
